...
| Project | Description | Stack | Link |
|---|---|---|---|
| **Nemotron Reasoning Challenge** | 🥈 Silver Medal, 114th place in the [NVIDIA Nemotron Model Reasoning Challenge](https://www.kaggle.com/competitions/nvidia-nemotron-model-reasoning-challenge) (Kaggle). The benchmark's puzzles (ciphers, numeral systems, unit conversion, bit manipulation, cryptarithms, equation deduction) are all deterministically solvable, so instead of trusting the base model's chain-of-thought I built a corpus of verifier-checked, faithful derivations from per-category solvers and a cryptarithm DSL, then fine-tuned a rank-32 LoRA over Nemotron-3-Nano-30B — plus synthetic augmentation and RFT self-distillation. | Python, PyTorch, LoRA | [Repo](https://github.com/MehulChourasia28/NVIDIA-NemotronKaggleChallenge) |
